...
char* HttpRequest::splitRequestLine(const char* start, const char* end, const char* sub, function<void(string)> callback)
{
    ...
    if (sub != nullptr)
    {
        // 这里搜索 method 和 url, version 因为 sub == nullptr, 所以不进入 if 
        space = static_cast<char*>(memmem(start, end - start, sub, strlen(sub)));
        assert(space != nullptr);
    }
    int length = space - start;
    // 这里的callback相当于调用了回调函数setMethod, urlFunc, versionFunc 初始化了m_method, m_url, m_version
...
void HttpRequest::sendDir(string dirName, Buffer* sendBuf, int cfd)
{
    ...
    // 使用 scandir 获取目录下的文件和子目录名，并通过 alphasort 按字母顺序排序。
    // num > 0：表示成功读取目录内容，返回目录中符合条件的文件和子目录的数量
    // 用于存储文件名数组的指针，成功后由系统分配，需手动释放。
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        struct stat st;
        char subPath[1024] = { 0 };
        // 将它们拼接成完整路径存储到 subPath，格式如 /path/to/dir/file.
        sprintf(subPath, "%s/%s", dirName.data(), name);
        stat(subPath, &st); // 返回0：成功，元信息保存在 st 中。
        if (S_ISDIR(st.st_mode)) // 如果是目录，则生成特定的 HTML 超链接，链接地址以 / 结尾。
        {
            // a标签 <a href="">name</a>
            ///strlen(buf) 计算的是缓冲区 buf 中已记录的 字符串 的长度。
            sprintf(buf + strlen(buf), // 
                "<tr><td><a href=\"%s/\">%s</a></td><td>%ld</td></tr>",
                name, name, st.st_size);
        }
        else // 如果是普通文件，则生成如下 HTML：
        {
            sprintf(buf + strlen(buf),
                "<tr><td><a href=\"%s\">%s</a></td><td>%ld</td></tr>",
                name, name, st.st_size);
...
void HttpRequest::sendFile(string fileName, Buffer* sendBuf, int cfd)
{
    ...
    while (1)
    {
        ...
        // buf: 一个指向存储读取数据的缓冲区的指针。read() 会把读取到的数据放在这个缓冲区里。
        // count : 要读取的最大字节数。通常是缓冲区的大小，以防止一次读取超过缓冲区的容量。    

...
EventLoop::EventLoop(const string threadName)
{
    ...
    m_dispatcher = new EpollDispatcher(this); // 选择一个dispatcher
    // map
    m_channelMap.clear();
    // 无论是 m_socketPair[0] 还是 m_socketPair[1]，都可以读写数据
    // m_socketPair 中只有两个文件描述符
    // 通过 m_socketPair[0] 写入数据时，m_socketPair[1] 可以读取该数据。
    // 同理，通过 m_socketPair[1] 写入数据时，m_socketPair[0] 可以读取该数据
    // 这种互相通信的特性，可能让人误以为多了一组文件描述符，但实际上这只是两个文件描述符的双向读写能力
    // 指定使用 UNIX 域套接字，数据在本地机器的进程间传输，不会走网络协议栈。
...
int EventLoop::processTaskQ()
{
    ...
    while (!m_taskQ.empty())
    {
        ...
        m_taskQ.pop();  // 删除节点(即从m_taskQ中删除刚刚取出的节点node)
        m_mutex.unlock();
...
{
    char buf[256];
    // 这行代码调用了 read 函数，用于从 m_socketPair[1] 套接字中读取数据到缓冲区 buf 中
    /*
        如果 m_socketPair[0] 没有写入数据：
        read(m_socketPair[1], buf, sizeof(buf)) 会阻塞，直到：
        有数据可供读取（通过 m_socketPair[0] 写入）。
        套接字关闭或出现其他错误。
        如果 m_socketPair[0] 写入了数据：
        read 会立即读取数据，并返回读取的字节数。
    */
    read(m_socketPair[1], buf, sizeof(buf)); // 注意：read 函数是阻塞的
    return 0;
}
